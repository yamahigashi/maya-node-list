...
def get_val(attr_type, plg, kwargs):
    val = ''
    try:
...
def consolidate_kids(arr):
    ...
    return od((k, v) for k, v in results.items() if not v['parent'])
...
def dump_node_id(z):
    ...
    try:
        ...
        dpn = om2.MFnDependencyNode()
        dpn = om2.MFnDependencyNode(dpn.create(id, 'test'))

        attributes = {}
        is_k_true = lambda k: k in kwargs and bool(kwargs[k])
        for num in xrange(ac):
            a = om2.MFnAttribute(dpn.attribute(num))
            plg = om2.MPlug(dpn.object(), a.object())

            info = plg.info
            name_long = a.name
            name_short = a.shortName
            add_cmd = a.getAddAttrCmd(True)
            # set_cmd = plg.getSetAttrCmds(om2.MPlug.kAll, True)

            flags = []
            is_internal = False
            kwargs = cmd_args_parser(add_cmd)

            attr_type = kwargs.get('attributeType', '')

            is_appear_cbox = a.channelBox
            is_extension = a.extension
            is_readable = a.readable
            is_writable = a.writable
            is_connectable = a.connectable
            is_hidden = a.hidden
            is_array = a.array
            is_storable = a.storable
            is_keyable = a.keyable

            is_internal = is_k_true('internalSet')
            is_internal = a.internal
            ...
            parent = None
            if plg.isChild:
                parent = om2.MFnAttribute(a.parent).name

            val = get_val(attr_type, plg, kwargs)
        ...
        attributes = od(sorted(attributes.items()))
        attributes = consolidate_kids(attributes)
        apper_in_cbox_attrs = od((k, v) for (k, v) in attributes.iteritems() if v['is_appear_cbox'])
        extern_attrs = od((k, v) for (k, v) in attributes.iteritems() if not v['is_internal'] and not v['is_appear_cbox'] and not v['is_hidden'])
        extern_hidden = od((k, v) for (k, v) in attributes.iteritems() if not v['is_internal'] and not v['is_appear_cbox'] and v['is_hidden'])
        internal_attrs = od((k, v) for (k, v) in attributes.iteritems() if v['is_internal'])

        plugin = dpn.pluginName or "_default"

        import json
        with open(r'{}\{}.json'.format(output_dir, hex(z)), 'w') as fp:
            json.dump(attributes, fp)

        d = r'{}\{}'.format(output_dir, os.path.basename(plugin))
        if not os.path.exists(d):
            os.makedirs(os.path.abspath(d))
        with open(r'{}\{}\{}.rst'.format(output_dir, os.path.basename(plugin), hex(z)), 'w') as fp:
            dat = rst_writer(
                z, nc, attributes,
                apper_in_cbox_attrs, internal_attrs, extern_attrs, extern_hidden=extern_hidden,
...
def rst_writer(id, node, attrs, apper_in_cbox_attrs, internal_attrs, extern_attrs, extern_hidden, **kwargs):
    results = tmpl.render(
        id=hex(id), node=node, attributes=attrs, apper_in_cbox_attrs=apper_in_cbox_attrs,
...
#        0x59475241,
    ...
    #dump_node_id(0x44414d58)
    # dump_node_id(0x4c4f4354)
    for id in idx:
        dump_node_id(id)
